...
public abstract class AbstractTemplateProcessor {
    ...
    public File process(TableDto data, boolean isWeb) throws IOException {
        ...
        logger.info("### 处理对象{}读取模板文件：{}，生成目标文件：{}。开始。。。", getClass().getSimpleName(), sourceConfig.getFileName(), targetConfig.getFileName());
        // 获取模板对象
        Template template = freemarkerProcessor.getTemplate(getGeneratorProperties().getTemplate().getBasePackagePath(), sourceConfig.getDirectory(), sourceConfig.getFileName());
        // 生成目标文件
        data.setBasePackage(generatorProperties.getTarget().getBasePackage());
        File build = build(data, template, targetConfig);
        logger.info("### 处理对象{}读取模板文件：{}，生成目标文件：{}。结束。。。", getClass().getSimpleName(), sourceConfig.getFileName(), build.getCanonicalPath());
        return build;
    }

    private File build(TableDto data, Template template, FileProperties target) {
